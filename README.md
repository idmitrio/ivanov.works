...
Автоматических тестов в проекте пока нет. `lint` и `build` проверяют код и
сборку, но не заменяют тесты.

## Основные маршруты

- `/` — главная страница;
- `/solutions` — каталог направлений автоматизации;
- `/solutions/<slug>` — сценарии «Было / Стало» для выбранного направления;
- `/privacy` — политика обработки персональных данных;
- `/case-template` — шаблон будущего клиентского кейса, а не опубликованный кейс.
...
- `app/` — страницы, общие стили и корневой layout;
- `app/site-header.tsx` — общий хедер и мобильное меню;
- `app/contact-modal.tsx` — единая форма «Обсудить процесс» и кнопки, которые её открывают;
- `app/solutions/` — каталог направлений, страницы сценариев и их данные;
- `app/yandex-metrika.tsx` — глобальный счётчик и учёт клиентских переходов между страницами;
- `app/api/contact/route.ts` — отправка заявок в Telegram;
